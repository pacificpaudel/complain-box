...
function doLogin() {
  ...
  if (email === SUPER_ADMIN) {
    let u = state.users.find(x => x.email === SUPER_ADMIN);
    if (!u) {
      state.users.push({ email: SUPER_ADMIN, password: hashStr(pass), name: 'Super Admin', role: 'super', orgId: null, createdAt: Date.now() });
      state.currentUser = { email: SUPER_ADMIN, name: 'Super Admin' };
      saveState(state); closeModal(); notify('Welcome, Super Admin!', 'success'); render(); return;
    }
    if (u.password !== hashStr(pass)) return notify('Wrong password', 'error');
    state.currentUser = { email: SUPER_ADMIN, name: u.name || 'Super Admin' };
    saveState(state); closeModal(); notify('Logged in as Super Admin', 'success'); render(); return;
  }

  const u = state.users.find(x => x.email === email);
  if (!u) return notify('Account not found', 'error');
  if (u.password !== hashStr(pass)) return notify('Wrong password', 'error');
  state.currentUser = { email: u.email, name: u.name || '' };
  saveState(state); closeModal(); notify('Welcome back!', 'success'); render();
}
...
function doRegister() {
  ...
  if (state.users.find(u => u.email === email)) return notify('Email already registered', 'error');

  const user = { email, password: hashStr(pass), name, role: orgId ? role : 'complainer', orgId: orgId || null, address: '', photo: '', createdAt: Date.now() };
  state.users.push(user);
  state.currentUser = { email, name };
  saveState(state); closeModal(); notify('Account created!', 'success'); render();
}
...
function doCreateOrg() {
  ...
  if (state.orgs.find(o => o.subdomain === subdomain)) return notify('Subdomain already taken', 'error');
  ...
  if (adminUser) {
    if (!state.users.find(u => u.email === adminEmail && u.orgId === orgId)) {
      state.users.push({ ...adminUser, role: 'admin', orgId });
    }
  } else {
    const tempPass = Math.random().toString(36).slice(2, 10);
    state.users.push({ email: adminEmail, password: hashStr(tempPass), name: adminEmail.split('@')[0], role: 'admin', orgId, address: '', photo: '', createdAt: Date.now() });
    notify(`Admin created. Temp password: ${tempPass} — note this down!`, 'info');
  }

  state.orgs.push({ id: orgId, name, subdomain, website, logo: '', sections: [...DEFAULT_SECTIONS], createdAt: Date.now() });
